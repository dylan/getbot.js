...
Pace.prototype.outputProgress = function outputProgress() {
    // Spinner addition
  this.charm.write(this.spinner[this.spinnerOffset]);
  if (this.spinnerOffset < this.spinner.length - 1) {
    this.spinnerOffset++;
  } else {
    this.spinnerOffset = 0;
  }
  ...
  this.charm.foreground(234).background(234);
  while (i < this.size - 1) {
    this.charm.write(' ');
    i++;
  }
...
Pace.prototype.outputStats = function outputStats() {
  ...
  this.total_len = this.Utils.toReadableSize(this.total).length;

  if(this.elapsed>1){
    var rate = this.current / this.elapsed * 1024;
  } else {
    var rate = 0;
  }
  this.charm.write('  ').display('bright').write(padLeft(this.Utils.toReadableSize(this.current), this.total_len)).display('reset');
  this.charm.write('/' + this.Utils.toReadableSize(this.total));
  this.charm.write('  ').display('bright').write(this.Utils.toReadableSize(rate)).display('reset').write('/s');
...
Pace.prototype.outputTimes = function outputTimes() {
  ...
  if (this.time_left){
    ...
    this.charm.write(hours + 'h ' + min + 'm ' + sec + 's').cursor(false);
  }
};
