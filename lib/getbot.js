...
(function() {
  ...
  Getbot = (function(_super) {
    ...
    function Getbot(opts) {
      ...
      this.origFilename = this.newFilename = this.fileExt = this.fileBasename = this.filename = this.path = null;
      if (this.destination) {
        ...
        fs.exists(this.destination, function(exists) {
          if (exists) {
            if (_this.destination.charAt(_this.destination.length - 1) === '/') {
              _this.startPath = process.cwd();
              process.chdir(_this.destination);
              if (!_this.forceOverwrite) {
                fs.exists(process.cwd() + '/' + _this.fileBasename + _this.fileExt, function(exists) {
                  if (exists) {
                    return _this.emit('fileExists', "" + (_this.destination + _this.fileBasename + _this.fileExt));
                  }
                });
              }
            } else {
              if (!_this.forceOverwrite) {
                _this.emit('fileExists', "" + _this.destination);
              }
              process.chdir(path.dirname(_this.destination));
            }
          } else {
            if (_this.destination.charAt(_this.destination.length - 1) === '/') {
              _this.startPath = process.cwd();
              nodeFS.mkdir(_this.destination, 0x1ff, true, function() {
                return process.chdir(_this.destination);
              });
            } else {
              if (path.dirname(_this.destination) !== '.') {
                _this.startPath = process.cwd();
                _this.path = path.dirname(_this.destination);
                nodeFS.mkdir(_this.path, 0x1ff, true, function() {
                  return process.chdir(_this.path);
                });
              }
            }
          }
          _this.filename = _this.origFilename = "" + _this.fileBasename + _this.fileExt;
          return _this.newFilename = "" + _this.origFilename + ".getbot";
        });
      } else {
        this.filename = decodeURI(url.parse(opts.address).pathname.split("/").pop());
        this.fileExt = path.extname(this.filename);
      ...
      req = request.head(reqOptions, function(error, response, body) {
        if (!error) {
              ...
              if (response.headers['content-length'] !== void 0 && response.headers['content-length'] !== 0) {
                ...
                try {
                  ...
                  return fs.open(_this.newFilename, 'w', function(err, fd) {
                    ...
                    return _this.startParts(reqOptions, _this.fileSize, _this.maxConnections, _this.download);
                  });
                } catch (error) {
