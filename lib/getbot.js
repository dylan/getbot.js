(function() {
  ...
  Getbot = (function(_super) {
    ...
    function Getbot(opts) {
      ...
      var options, req,
        _this = this;
      this.downloadStart = this.fileSize = this.partsCompleted = this.maxConnections = 0;
      this.fileExt = this.fileBasename = this.newFilename = this.fileDescriptor = null;
      this.destination = opts.destination;
      this.listDownload = opts.list;
      ...
      } else if (this.destination && this.listDownload) {
        this.filename = decodeURI(url.parse(opts.address).pathname.split("/").pop());
        this.dir = this.destination;
        this.startPath = process.cwd();
        fs.mkdir(this.dir, function() {
          return process.chdir(_this.dir);
        });
      } else {
    ...
    Getbot.prototype.download = function(options, offset, end, number) {
      ...
      return req = request(options, function(error, response) {
        ...
        if (_this.partsCompleted === _this.maxConnections) {
          file.end();
          return fs.rename(_this.newFilename, _this.filename, function() {
