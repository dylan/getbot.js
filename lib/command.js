(function() {
  ...
  exports.run = function() {
    ...
    program.version('0.0.6b').usage('[options] <URL>').option('-d, --destination [path]', 'the destination for the downloaded file').option('-c, --connections [number]', 'max connections to try', parseInt, 5).option('-u, --user [string]', 'username used for basic http auth').option('-p, --pass [string]', 'password used for basic http auth').option('-l, --list [string]', 'a list of urls (one on each line) to read in and download from').parse(process.argv);
